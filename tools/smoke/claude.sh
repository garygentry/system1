...
drive() { # $1 = workdir, $2 = prompt, $3 = output file
  (cd "$1" && timeout "$TIMEOUT" claude -p --plugin-dir "$REPO/plugins/decisions" \
    --allowedTools "Bash(decide *)" "Skill" --model "${SMOKE_CLAUDE_MODEL:-haiku}" "$2" </dev/null) >"$3" 2>&1 || true
}
status=0
mkdir -p "$SMOKE/claude"
drive "$SMOKE/claude" "$PING_PROMPT" "$SMOKE/claude/ping.txt"
assert_marker claude:ping "$PING_MARKER" "$SMOKE/claude/ping.txt" || status=1
