...
- **CLI contract** ([0015](plans/decisions/0015-cli-contract-v1.md)): one JSON envelope `{v, ok, command, result|error}` per command; exit codes 0 ok, 1 bug, 2 usage, 3 egress refused, 4 budget guard, 5 provider, 6 replay miss, 7 `spec check --strict` mismatch. Tools are defined once in `packages/core/src/tools/` (TypeBox input schema + handler); the CLI only maps argv and formats. A breaking change bumps `ENVELOPE_VERSION` and needs a decision record.
