# AGENTS.md — working on the `system1` repo
...
| Path | What it is |
|---|---|
| `packages/core` | `@garygentry/system1-core`, the engine library (model profiles, transport, sources, fixtures, spend) |
...
- **CLI contract** ([0015](plans/decisions/0015-cli-contract-v1.md)): one JSON envelope `{v, ok, command, result|error}` per command; exit codes 0 ok, 1 bug, 2 usage, 3 egress refused, 4 budget guard, 5 provider, 6 replay miss, 7 a check that did not pass (`spec check --strict` mismatch, `spec lint` failure). Tools are defined once in `packages/core/src/tools/` (TypeBox input schema + handler); the CLI only maps argv and formats. A breaking change bumps `ENVELOPE_VERSION` and needs a decision record.
...
- **Claude Code** puts plugin `bin/` on the Bash PATH, so `decide` resolves to the generated shim. Test with `claude -p --plugin-dir plugins/system1`.
