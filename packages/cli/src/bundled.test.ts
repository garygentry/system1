...
function tree(files: string[]): string {
  ...
  for (const f of files) {
    ...
    if (f.endsWith("/")) mkdirSync(join(root, f), { recursive: true })
      ...
      writeFileSync(
        join(root, f),
        f.endsWith("package.json") ? JSON.stringify({ name: "@garygentry/decisions" }) : "",
      )
  }
  return root
}
...
describe("bundledSpecsDir", () => {
  ...
  it("uses the plugin's specs in a checkout, from tsc or bundle output", () => {
    ...
    const want = join(root, "plugins/decisions/specs")
  ...
  it("ignores a look-alike folder next to an installed package", () => {
    // node_modules/@garygentry/decisions → ../.. is node_modules, which has no catalog.yaml.
    const root = tree([
      "node_modules/plugins/decisions/specs/",
      "node_modules/@garygentry/decisions/package.json",
    ])
    const from = join(root, "node_modules/@garygentry/decisions/dist/bundle/chunks/x.mjs")
