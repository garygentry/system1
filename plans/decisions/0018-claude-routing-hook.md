...
- **It never gets in the way.** The hook prints only when `decide` exits 0, always exits 0 itself (exit 2 from `UserPromptSubmit` would block the prompt), sets `SYSTEM1_NO_NPX` so the shim never downloads the CLI mid-prompt, and has a 10 s timeout. Without a global install, the shim runs the pinned version from npx's cache instead (added in 0.3.1). Before that change, a user who had only the plugin, the documented Claude install, got no hints at all. The evals missed it because they run the checkout bundle. `route` loads config and regexes only, not the tool context, so it adds little to each prompt.
