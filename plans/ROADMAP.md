...
| **M5** | Skills: ad-hoc asking, saved specs, setup | **in progress**, see `milestones/M5-skills-specs.md`. `ask` (with the craft references), `design` (save and repair repo specs), `setup` (user-only, consent-gated). `--questions` and `spec check`. Routing evals in Claude, Codex and Pi. No bundled specs |
